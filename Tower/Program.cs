...
{
    // Habilita a documentação de anotaçoes do Swagger
    c.EnableAnnotations();

    // Define a informação básica da API
    c.SwaggerDoc("v1", new OpenApiInfo
    {
        Title = "API de consultas Mister Multas",
    });

    // Define o esquema de segurança do token JWT
    ...
    // Configuraçãoo para evitar conflitos de rotas
    c.ResolveConflictingActions(x => x.First());

    // Habilita as anotações para herança e polimorfismo
    c.EnableAnnotations(enableAnnotationsForInheritance: true, enableAnnotationsForPolymorphism: true);

    // Configuração para incluir a documentação XML da API
